<template>
  <nav aria-label="pagination">
    <ul class="p-0 m-0">
      <li v-show="pages > 5 && model > 3" type="button" class="btn btn-white btn-med box-shadow-med arrow"
        @click="model--"
      >
      ...
      <li
        ...
        @click="model = i + 1"
        :tabindex="i+1 === model ? 0 : -1"
        :aria-label="`page ${i+1}`" :aria-checked="i+1 === model"
      >
        {{ i + 1 }}
      </li>
      <li
      ...
      <li v-show="pages > 5 && model < pages - 3" type="button" class="btn btn-white btn-med box-shadow-med arrow btn-white"
        @click="model++"
      >

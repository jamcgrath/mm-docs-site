<template>
	<div
		class="mm-double-slider-wrapper"
		role="group"
		aria-labelledby="mm-double-slider-label"
		:id="id || randomId"
	>
		<div id="mm-double-slider-label" class="visually-hidden">{{ label }}</div>
		<div
			class="mm-double-slider"
			:class="disabled && 'disabled'"
			:style="styles"
		>
			<div
				class="mm-double-slider-progress"
				:class="disabled && 'disabled'"
				:id="`progress_${id || randomId}`"
			></div>
		</div>
		<div class="mm-double-silder-inputs">
			<span
				class="mm-double-slider-tooltip label"
				:id="`tooltip_${id || randomId}_from`"
				v-if="displayTooltip"
				v-show="showTooltip1"
				>{{ value[0] }}</span
			>
			<input
				type="range"
				class="mm-double-silder-input"
				:min="min"
				:max="max"
				:step="step"
				:value="value[0]"
				:id="`input_${id || randomId}_from`"
				:disabled="disabled"
				@input="sliderProgress('from')"
				aria-label="minimum value"
				@focus="showTooltip1 = true"
				@blur="showTooltip1 = false"
			/>
			<span
				class="mm-double-slider-tooltip label"
				:id="`tooltip_${id || randomId}_to`"
				v-if="displayTooltip"
				v-show="showTooltip2"
				>{{ value[1] }}</span
			>
			<input
				...
				:step="step"
				:value="value[1]"
				:id="`input_${id || randomId}_to`"
				:disabled="disabled"
				@input="sliderProgress('to')"
		...
		mounted() {
			...
			this.$nextTick(() => {
				...
				if (this.displayTooltip) {
					const sliderFrom = document.getElementById(
						`input_${this.id || this.randomId}_from`
					)
					const sliderTo = document.getElementById(
						`input_${this.id || this.randomId}_to`
					)
		...
		methods: {
			sliderProgress(type) {
				...
				const progress = document.getElementById(
					`progress_${this.id || this.randomId}`
				)
				...
				if (this.displayTooltip) {
					const tooltipFrom = document.getElementById(
						`tooltip_${this.id || this.randomId}_from`
					)
					const tooltipTo = document.getElementById(
						`tooltip_${this.id || this.randomId}_to`
					)

					tooltipFrom.style.left = `calc(${fillValueFrom}% + (${
						6.15 - fillValueFrom * 0.15
					}px))`
					tooltipTo.style.left = `calc(${fillValueTo}% + (${
						6.15 - fillValueTo * 0.15
					}px))`
				...
				this.$emit('input', [
					parseFloat(sliderFrom.value),
					parseFloat(sliderTo.value),
				])
			},
			tooltipTimeout() {
